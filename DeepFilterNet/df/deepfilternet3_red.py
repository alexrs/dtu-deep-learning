...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        e3 = self.erb_conv3(e2)  # [B, C*8, T, F/8]
...
class ErbDecoder(nn.Module):
    def __init__(self):
        ...
        self.conv0_out = conv_layer(
            p.conv_ch, 1, kernel_size=p.conv_kernel_inp, activation_layer=nn.Sigmoid
        )
    ...
    def forward(self, emb, e3, e2, e1, e0) -> Tensor:
        ...
        e3 = self.convt3(e3 + emb)  # [B, C*4, T, F/4]
        e2 = self.convt2(e2 + e3)  # [B, C*2, T, F/2]
        e1 = self.convt1(e1 + e2)  # [B, C, T, F]
        m = self.conv0_out(e0 + e1)  # [B, 1, T, F]
        return m
...
class DfDecoder(nn.Module):
    def __init__(self):
        ...
        df_fc_emb = GroupedLinearEinsum(
            p.conv_ch * p.nb_df, self.emb_in_dim, groups=p.enc_lin_groups
        )
        self.df_fc_emb = nn.Sequential(df_fc_emb, nn.ReLU(inplace=True))

        self.df_fc = nn.Sequential(nn.Linear(self.emb_in_dim, 128), nn.ReLU(inplace=True))
    ...
    def forward(self, feat_spec: Tensor, emb: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        cemb = c0.permute(0, 2, 3, 1).flatten(2)  # [B, T, -1]
        cemb = self.df_fc_emb(cemb)  # [T, B, C * F/4]
        cemb = self.df_fc(cemb)
        c, _ = self.df_gru(emb + cemb)  # [B, T, H], H: df_n_hidden
        ...
        c = c.view(b, t, self.df_bins, self.df_out_ch)  # [B, T, F, O*2]
        return c, alpha
...
class DfNet(nn.Module):
    ...
    def forward(
        ...
        e0, e1, e2, e3, emb, lsnr = self.enc(feat_erb)
        if self.run_erb:
        ...
        if self.run_df:
            df_coefs, _ = self.df_dec(feat_spec, emb)
